...
import { ChevronRight, ChevronLeft, Check, Upload, AlertTriangle, ShieldCheck, UserPlus, CreditCard, Loader2, Plus, Trash2, FileText } from 'lucide-react';
...
const CandidateRegistration: React.FC = () => {
  ...
  if (isSubmitted) {
    return (
      <div className="min-h-screen pt-24 pb-12 px-6 flex items-center justify-center">
        <div className="bg-midnight/60 border border-green-500/30 rounded-3xl p-8 max-w-lg text-center backdrop-blur-xl">
          <div className="w-20 h-20 bg-green-500/20 rounded-full flex items-center justify-center mx-auto mb-6">
            <ShieldCheck size={40} className="text-green-500" />
          </div>
          <h2 className="text-3xl font-bold text-white mb-4">Registration Received</h2>
          <p className="text-slate-300 mb-6">
            Thank you, {formData.firstName}. Your profile has been submitted to Jackson Multifacet. 
            Our recruitment team will verify your payment and details shortly.
          </p>
  ...
  return (
    <div className="min-h-screen pt-24 pb-20 relative z-10" ref={topRef}>
      <div className="max-w-4xl mx-auto px-6">
        {/* Header */}
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-cyan/10 border border-cyan/30 text-cyan text-xs font-bold uppercase tracking-widest mb-4">
            Portal 2026
          </div>
          <h1 className="text-3xl md:text-5xl font-bold text-white mb-4">Candidate Registration</h1>
          <p className="text-slate-400">Join the elite talent pool at Jackson Multifacet (Real Value & Stakes Ltd).</p>
        </div>
        ...
        {/* Form Container */}
        <motion.div 
          className="bg-midnight/40 border border-white/10 backdrop-blur-xl rounded-3xl p-6 md:p-10 shadow-2xl relative overflow-hidden"
        ...
        >
          ...
          {step === 1 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              ...
              <div className="bg-white/5 rounded-xl p-6 space-y-4 max-h-[400px] overflow-y-auto text-sm text-slate-300">
          ...
          {step === 2 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Input label="Surname" value={formData.surname} onChange={v => handleInputChange('surname', v)} required />
                <Input label="First Name" value={formData.firstName} onChange={v => handleInputChange('firstName', v)} required />
                <Input label="Other Name" value={formData.otherName} onChange={v => handleInputChange('otherName', v)} />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <Input label="Date of Birth" type="date" value={formData.dob} onChange={v => handleInputChange('dob', v)} required />
                 <Select label="Sex" value={formData.sex} onChange={v => handleInputChange('sex', v)} options={['M', 'F']} required />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <Input label="Nationality" value={formData.nationality} onChange={v => handleInputChange('nationality', v)} />
                 <Input label="State of Origin" value={formData.stateOfOrigin} onChange={v => handleInputChange('stateOfOrigin', v)} required />
                 <Input label="L.G.A" value={formData.lga} onChange={v => handleInputChange('lga', v)} required />
                 <Input label="Religion" value={formData.religion} onChange={v => handleInputChange('religion', v)} />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <Input label="Phone Number" type="tel" value={formData.tel} onChange={v => handleInputChange('tel', v)} required />
                 <Input label="WhatsApp" type="tel" value={formData.whatsapp} onChange={v => handleInputChange('whatsapp', v)} />
                 <Input label="Email Address" type="email" value={formData.email} onChange={v => handleInputChange('email', v)} required />
                 <Select label="Marital Status" value={formData.maritalStatus} onChange={v => handleInputChange('maritalStatus', v)} options={['Single', 'Married', 'Divorced', 'Widowed']} />
              </div>

              <Input label="Residential Address" value={formData.address} onChange={v => handleInputChange('address', v)} required />
              ...
              <div className="p-4 bg-white/5 rounded-xl border border-white/10">
                ...
                {formData.handicap === 'Yes' && (
                  <Input label="If yes, please describe:" value={formData.handicapDescription || ''} onChange={v => handleInputChange('handicapDescription', v)} />
                )}
              </div>
              ...
              <div className="mt-8 pt-6 border-t border-white/10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Input label="Full Name" value={formData.nextOfKin.name} onChange={v => handleNestedChange('nextOfKin', 'name', v)} required />
                  <Input label="Relationship" value={formData.nextOfKin.relationship} onChange={v => handleNestedChange('nextOfKin', 'relationship', v)} required />
                  <Input label="Phone" value={formData.nextOfKin.tel} onChange={v => handleNestedChange('nextOfKin', 'tel', v)} required />
                  <Input label="Email" value={formData.nextOfKin.email} onChange={v => handleNestedChange('nextOfKin', 'email', v)} />
                  <div className="md:col-span-2">
                    <Input label="Address" value={formData.nextOfKin.address} onChange={v => handleNestedChange('nextOfKin', 'address', v)} required />
                  </div>
                </div>
              </div>
          ...
          {step === 3 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                {[0, 1, 2].map(i => (
                  <Input key={i} label={`Position Preference ${i + 1}`} value={formData.desiredPositions[i]} onChange={v => handleArrayChange('desiredPositions', i, v)} placeholder={i === 0 ? "e.g. Sales Manager" : ""} />
                ))}
              </div>
              ...
              <div className="space-y-4 mt-6">
                ...
                {[0, 1, 2].map(i => (
                  <Input key={i} label={`Location Preference ${i + 1}`} value={formData.jobLocations[i]} onChange={v => handleArrayChange('jobLocations', i, v)} placeholder={i === 0 ? "e.g. Lagos Island" : ""} />
                ))}
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                <Select label="Preferred Job Mode" value={formData.jobMode} onChange={v => handleInputChange('jobMode', v)} options={['On-Site', 'Remote', 'Hybrid']} required />
                <Input label="Years of Experience" type="number" value={formData.yearsExperience} onChange={v => handleInputChange('yearsExperience', v)} />
              </div>
              ...
              <div className="p-6 bg-indigo-900/10 border border-indigo-500/30 rounded-xl mt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   <Select label="ID Type" value={formData.idType} onChange={v => handleInputChange('idType', v)} options={['National ID (NIN)', "Voter's Card", "Driver's License", "International Passport"]} required />
                   <Input label="ID Number" value={formData.validIdNumber} onChange={v => handleInputChange('validIdNumber', v)} required />
                </div>
              </div>
              ...
              {[0, 1].map((index) => (
                <div key={index} className="bg-white/5 p-6 rounded-xl border border-white/10 relative">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <Input label="Full Name" value={formData.guarantors[index].name} onChange={v => handleGuarantorChange(index, 'name', v)} required />
                    <Input label="Relationship" value={formData.guarantors[index].relationship} onChange={v => handleGuarantorChange(index, 'relationship', v)} required />
                    <Input label="Phone" value={formData.guarantors[index].tel} onChange={v => handleGuarantorChange(index, 'tel', v)} required />
                    <Input label="Email" value={formData.guarantors[index].email} onChange={v => handleGuarantorChange(index, 'email', v)} />
                    <Input label="Occupation" value={formData.guarantors[index].occupation} onChange={v => handleGuarantorChange(index, 'occupation', v)} required />
                    <Input label="Years Known" value={formData.guarantors[index].yearsKnown} onChange={v => handleGuarantorChange(index, 'yearsKnown', v)} required />
                    <div className="md:col-span-2">
                       <Input label="Residential Address" value={formData.guarantors[index].address} onChange={v => handleGuarantorChange(index, 'address', v)} required />
                    </div>
                     <div className="md:col-span-2">
                       <Input label="Work Address" value={formData.guarantors[index].workAddress} onChange={v => handleGuarantorChange(index, 'workAddress', v)} required />
                    </div>
                  </div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              ...
              <div className="space-y-4 pt-6">
                 ...
                 <Input 
                   ...
                   onChange={v => handleInputChange('paymentReference', v)} 
